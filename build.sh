...
if [ -z "$2" ] || [ $2 = "setup" ]; then
    ...
    snarkjs groth16 setup compilers/$NAME.r1cs groth16/powersOfTau28_hez_final_15.ptau groth16/"$NAME"_0000.zkey
...
if [ -z "$2" ] || [ $2 = "verify" ]; then
    ...
    if output=$(npx snarkjs groth16 prove groth16/"$NAME"_0000.zkey compilers/"$NAME"_js/witness.wtns proofs/$NAME/proof.json proofs/$NAME/public.json 2>&1); then
        snarkjs groth16 verify groth16/verification_key.json proofs/$NAME/public.json proofs/$NAME/proof.json
    else
        echo "$output"
    fi
fi